...
initd (void *f_name) {
	...
	// /**/printf("------- process_create_initd end -------\n");
	NOT_REACHED ();
}
 ...
 *
 * - UPAGE에 있는 READ_BYTES 바이트는 OFS에서 시작하여 FILE에서 읽어야 합니다.
 *
 * - UPAGE + READ_BYTES에 있는 ZERO_BYTES 바이트는 0으로 초기화해야 합니다.
 *
 * 이 함수로 초기화된 페이지는 WRITABLE이 true인 경우 사용자 프로세스가 쓰기가 가능해야 하고,
...
setup_stack (struct intr_frame *if_) {
	bool success = false;
	void *stack_max = (void *) (((uint8_t *) USER_STACK) - PGSIZE);

	/* TODO: stack_max에 스택을 매핑하고 페이지를 즉시 할당하세요.
